...
test('online shell requests prefer the network and refresh the offline cache',async()=>{
  const source=await readFile(new URL('../../src/client/sw.js',import.meta.url),'utf8');
  const listeners=new Map();
  const writes=[];
  let networkCalls=0;
  const stale={body:'old styles'};
  const fresh={body:'new styles',ok:true,clone(){return {...this}}};
  const cache={
    addAll:async()=>{},
  ...
  runInNewContext(source,{self,caches,fetch,location:{origin:'https://seat.test'},URL,Promise});
  let responsePromise;
  listeners.get('fetch')({
    request:{url:'https://seat.test/client/styles.css'},
    respondWith:promise=>{responsePromise=Promise.resolve(promise);}
  });

  const response=await responsePromise;
  assert.equal(response.body,'new styles');
  assert.equal(networkCalls,1);
  assert.deepEqual(writes,['new styles']);
});
