...
<html lang="zh-Hant">
  ...
  <body>
    ...
    <main>
      ...
      <section id="light-meals" class="light-meals-section"><div class="section-heading compact"><p class="eyebrow">不只火鍋</p><h2>台式招牌飯麵</h2><p>想吃得快又有飽足感，可選台式飯麵。</p></div><div class="light-meal-grid">${homeMeals.map(([slug, name, price, description]) => `<article class="light-meal-card"><img src="/assets/light-meals/${slug}-1024.webp" srcset="/assets/light-meals/${slug}-480.webp 480w, /assets/light-meals/${slug}-1024.webp 1024w" sizes="(max-width: 560px) 46vw, (max-width: 1100px) 45vw, 30vw" alt="鼎鑽火鍋${name}" width="1024" height="704" loading="lazy" decoding="async" /><div><h3>${name}</h3><strong>${price}</strong><p>${description}</p></div></article>`).join("")}</div><a class="primary-action" href="/zh-hant/menu/">查看完整菜單</a></section>
